...
<%@page import="iCore.modelDAO.DAO_ThongBao"%>
...
<%
	...
	String[] tk_value = {"maTB", "tenTB", "ngayTao", "link", "noiDung"};
	String[] tk_show = {"Mã thông báo", "Tên thông báo", "Ngày tạo", "Link", "Nội dung"};
%>

<%@ include file="../../iPartial/code-header.jsp"%>

<%
	ObjectDAO<ThongBao> dao = new DAO_ThongBao();
	...
	if (session.getAttribute("checkTimKiem") != null) {
		...
		if (listTemp.size() > 0) {
			if (listTemp.get(0) instanceof ThongBao) {
				list = (ArrayList<ThongBao>) listTemp;
			} else {
	...
	<div class="col-lg-12">
		...
		<div class="panel-body">
			...
			<table width="100%"
				...
				<tbody>
					<%
						for (ThongBao obj : list) {
							//Bat buoc co de bo vao doan code xem chi tiet, chinh sua va xoa
							String maDoiTuong = obj.getMaTB();
							String tenDoiTuong = obj.getTenTB();
					%>
					<tr class="odd gradeX">
						<td><%=obj.getMaTB()%></td>
						<td><%=obj.getTenTB() != null ? obj.getTenTB() : ""%></td>
						<td><%=obj.getNhanVien() == null ? "" : obj.getNhanVien().getTenNV()%></td>
						<td><%=obj.getLink() != null ? obj.getLink() : ""%></td>
