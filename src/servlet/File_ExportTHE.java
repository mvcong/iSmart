...
/**
 * Servlet implementation class File_ExportTHE
 */
@WebServlet("/Servlet_FileExportTHE")
public class File_ExportTHE extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public File_ExportTHE() {
        super();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String tenFile = "DANH_SACH_THE " + dateString + "".concat(".xls");
		// tao file excel
		HSSFWorkbook myWorkBook = new HSSFWorkbook();
		
		ArrayList<TheThanhVien>thethanhviens = new DAO_TheThanhVien().listAll();
		
		CreateSheet(myWorkBook, thethanhviens);
	...
	private void CreateSheet(HSSFWorkbook myWorkBook, ArrayList<TheThanhVien> thethanhviens) {
			...
			Cell cell;
			Row row;
			for (int i = 0; i < thethanhviens.size(); i++) {
				TheThanhVien the = thethanhviens.get(i);
				...
				cell = row.createCell(1, CellType.STRING);
				cell.setCellValue(the.getMaThe());
				cell.setCellStyle(styleConten);

				cell = row.createCell(2, CellType.STRING);
				cell.setCellValue(the.getNgayTao());
				cell.setCellStyle(styleConten);

				cell = row.createCell(3, CellType.STRING);
				cell.setCellValue(the.getNgayKT());
				cell.setCellStyle(styleConten);
				
				cell = row.createCell(4, CellType.STRING);
				cell.setCellValue(the.getGioBD());
				cell.setCellStyle(styleConten);
				
				cell = row.createCell(5, CellType.STRING);
				cell.setCellValue(the.getGioKT());
				cell.setCellStyle(styleConten);
				
				cell = row.createCell(6, CellType.STRING);
				cell.setCellValue(the.getLoaiThe());
				cell.setCellStyle(styleConten);
				
				
				cell = row.createCell(7, CellType.STRING);
				cell.setCellValue(the.getThanhVien().getTenTV());
				cell.setCellStyle(styleConten);
