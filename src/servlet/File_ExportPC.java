...
public class File_ExportPC extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		ArrayList<PhieuChi>phieuchis = new DAO_PhieuChi().listAll();
		
		CreateSheet(myWorkBook, phieuchis);
	...
	/*	row = mySheet.createRow(0);
		...
		cell = row.createCell(1, CellType.STRING);
		cell.setCellValue("Mã phiếu chi");
		cell.setCellStyle(title);

		cell = row.createCell(2, CellType.STRING);
		cell.setCellValue("Tên phiếu chi");
		cell.setCellStyle(title);
	...
	private void CreateSheet(HSSFWorkbook myWorkBook, ArrayList<PhieuChi> phieuchis) {
			...
			Cell cell;
			Row row;
			for (int i = 0; i < phieuchis.size(); i++) {
				PhieuChi pc = phieuchis.get(i);
				...
				cell = row.createCell(1, CellType.STRING);
				cell.setCellValue(pc.getMaPC());
				cell.setCellStyle(styleConten);

				cell = row.createCell(2, CellType.STRING);
				cell.setCellValue(pc.getTenPC());
				cell.setCellStyle(styleConten);

				cell = row.createCell(3, CellType.STRING);
				cell.setCellValue(pc.getThanhPhan());
				cell.setCellStyle(styleConten);
				
				cell = row.createCell(4, CellType.STRING);
				cell.setCellValue(pc.getSoLuong());
				cell.setCellStyle(styleConten);
					
				cell = row.createCell(5, CellType.STRING);
				cell.setCellValue(pc.getDonGia());
				cell.setCellStyle(styleConten);
					
				cell = row.createCell(6, CellType.STRING);
				cell.setCellValue(pc.getThanhTien());
				cell.setCellStyle(styleConten);
					
				
				cell = row.createCell(7, CellType.STRING);
				cell.setCellValue(pc.getNhanVien().getTenNV());
				cell.setCellStyle(styleConten);
				rowst++;
